...
test_that("target_mae requires a regression target", {
  
  expect_error(target_mae(class_target))
  
})

test_that("target_mae returns a non-negative tibble", {
  
  result <- target_mae(reg_target)
  
  expect_identical(names(result), c("source", "on", "mae"))
