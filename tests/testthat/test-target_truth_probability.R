...
# target_truth_probability ---------------------------------------

test_that("target_truth_probability requires a classification target", {
  
  expect_error(target_truth_probability(reg_target))
  
})

test_that("target_truth_probability returns a tibble in [0, 1]", {
  
  result <- target_truth_probability(class_target)
  
  expect_identical(names(result), c("source", "on", "truth_probability"))
  expect_true(result$truth_probability >= 0 && result$truth_probability <= 1)
