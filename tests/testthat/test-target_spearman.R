...
test_that("target_spearman requires a regression target", {
  
  expect_error(target_spearman(class_target))
  
})

test_that("target_spearman returns a tibble in [-1, 1]", {
  
  result <- target_spearman(reg_target)
  
  expect_identical(names(result), c("source", "on", "spearman"))
