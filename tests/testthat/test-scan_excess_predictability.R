...
test_that("scan_excess_predictability basic functionality", {
  res <- scan_excess_predictability(toy_scan_holdout)

  expect_identical(
    names(res),
    c("target_var", "synthetic", "holdout", "excess_predictability")
  )

  # from test-scan_predictability.R: synthetic predictability = 4/5, holdout = 1
  expect_equal(res$synthetic, 4 / 5)
  expect_equal(res$holdout, 1)
  expect_equal(res$excess_predictability, 4 / 5 - 1)

})

test_that("scan_excess_predictability errors without holdout data", {
  expect_error(scan_excess_predictability(toy_scan_no_holdout))
})
