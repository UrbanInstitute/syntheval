test_that("discrimination() returns the correct object " , {
  ...
  discrimination <- discrimination(penguins_syn, penguins_conf)
  
  expect_equal(nrow(discrimination$combined_data), nrow(penguins_syn$synthetic_data) + nrow(penguins_conf))
  
  expect_equal(ncol(discrimination$combined_data), ncol(penguins_syn$synthetic_data) + 1)
