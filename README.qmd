...
The goal of syntheval is to make it simple to evaluate the utility and disclosure risk of synthetic data. The package works with `library(tidysynthesis)`.

## Installation

You can install the development version of syntheval by cloning this respository and building the package in RStudio. 
...
`$ci_overlap()` summarizes each coefficient including how much the confidence intervals overlap, if the signs match, and if the statistical significances matche. 
...
* By default, the code creates a training/testing split. This can be turned off with `split = FALSE`.
...
Next, we use `library(tidymodels)` to specify a model. We recommend the [tidymodels tutorial](https://www.tidymodels.org/start/) to leanr more. 
...
* `add_discriminator_auc()` to add the ROC AUC for the predicted probabilites
