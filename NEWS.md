...
- Add `print()` and `summary()` methods for `discrimination` objects. `summary()` returns all computed discriminant metrics in one tibble with columns `.metric`, `.sample`, and `.value`. (#111)
...
* Add GLMs to confidence interval overlap in `util_ci_overlap()`. (#128)
* Add `util_constraints()` to evaluate how well the confidential, synthetic, and holdout data meet pre-specified constraints. (#25)
